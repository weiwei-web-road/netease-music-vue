...
<template>
  <div class="personal-study">
    ...
    <div class="body">
      <multiSelect />
      <footerPagination class="common-padding" :total-list-num="100" />
      <limitDoubleDateSearch
        ...
        :get-data="changeDate"
      />
      <radioSelect class="common-padding" />
      <elTooltipHover class="common-padding" />
      <timePicker :init-time="initTime" :get-data="getSelectedTime" />
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    convertStrToArr() {
      ...
      let arr;
      const res = [];
      const value = '["baoma","chenxin"]';
      if (value.length > 4) {
        str = value.substring(1, value.length - 1);
        arr = str.split(",");
        arr.forEach((item) => {
          const temp = item.substring(1, item.length - 1);
          res.push(temp);
        });
      }
      return res;
